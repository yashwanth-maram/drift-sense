...
### When it fails, why it fails, and how it knows.
...
![failures](https://img.shields.io/badge/failures-2%20of%20100-f85149?style=for-the-badge&labelColor=0d1117)
![flagged](https://img.shields.io/badge/failures%20flagged%20by%20confidence-2%20of%202-3fb950?style=for-the-badge&labelColor=0d1117)
![rejected](https://img.shields.io/badge/approaches%20rejected-16-8b949e?style=for-the-badge&labelColor=0d1117)
...
>
> This document is that — plus five oracle experiments, sixteen rejected
> approaches, and three results we published internally and then withdrew.
...
| § | Section | In one line |
|:--|:--|:--|
| [1](#1--headline-result) | Headline result | 98.0 %, two failures |
| [2](#2--the-failure-mode) | The failure mode | strip-free references, lattice-multiple errors |
| [3](#3--worked-example-sample-49) | Worked example | 156.84 px, confidence 0.000 |
| [4](#4--predicting-failure-before-it-happens) | Predicting failure | detectable from the reference alone |
| [5](#5--degradation-under-noise) | Degradation under noise | and why the confidence score is honest |
| [6](#6--what-was-ruled-out-by-oracle) | Ruled out by oracle | perfect information, no change |
| [7](#7--rejected-approaches) | Rejected approaches | sixteen, each with a mechanism |
| [8](#8--retracted-results) | Retracted results | three, plus two analysis errors |
| [9](#9--known-limitations) | Known limitations | stated plainly |

---

# 1 · Headline result

Measured on **100 pairs** from the reference generator supplied with the
problem statement, using the shipped `localize.py`.

| | Drift-Sense | Plain matching |
|:--|--:|--:|
...
Two failures in a hundred. Both are dissected below.
...
> [!NOTE]
> **Nothing lands between 2 px and 50 px.** The method either finds the
> correct site to sub-pixel precision or lands in a different mat
> entirely. There is no intermediate regime of slightly-wrong answers to
> recover, which is why sub-pixel refinement alone was measured to change
> nothing.

Under plain matching, before the resampling operator was added, error
magnitudes fell at integer multiples of the lattice pitch — 9.8, 9.2, 38.9
(= 4 × 9.7), 144.6 (= 15 × 9.6). The matcher was landing on the right
lattice, at the right phase, and **miscounting periods**.
...
> [!CAUTION]
> This is a property of the **data**, not of the algorithm. A crop from a
> mat interior contains no aperiodic feature, so nothing distinguishes it
> from the same crop displaced by whole periods.
...
Taking the maximum over a family of templates at sub-pixel offsets restores
it.
...
> [!IMPORTANT]
> **It did not fail silently.** Confidence came out at **0.000**, against a
> median of 0.105 on correct predictions.
>
> In an inspection tool that distinction is the whole point: a tool that
> knows it missed can re-acquire, whereas a confidently wrong tool measures
...
The condition that causes failure is detectable from the reference image
alone, **before any matching runs**.

```mermaid
flowchart TD
    R["<b>Reference image</b><br/>1000 × 1000"]
    D{"Peripheral strip<br/>present on each axis?"}
    B["<b>Both axes pinned</b><br/>27 % of samples"]
    O["<b>One axis pinned</b><br/>61 % of samples"]
    N["<b>Neither pinned</b><br/>12 % of samples"]
    B2["96.3 % accurate"]
    O2["100.0 % accurate"]
    N2["91.7 % accurate<br/><i>8.3 % under plain matching</i>"]
    ...
    style R fill:#161b22,stroke:#2f81f7,color:#e6edf3
    style D fill:#1c2128,stroke:#8b949e,color:#e6edf3
    style B2 fill:#0d2818,stroke:#3fb950,color:#e6edf3
    style O2 fill:#0d2818,stroke:#3fb950,color:#e6edf3
    style N2 fill:#2d1618,stroke:#f85149,color:#e6edf3
```

A strip is a flat, low-variance band roughly 32 px wide in search
coordinates — the peripheral circuitry separating sub-array mats, and the
only aperiodic landmark in the field.

Under plain matching that is an **11× spread**, known in advance. The
shipped method largely closes it, and the predictor still identifies where
...
> [!WARNING]
> Applied Materials state that the test set will be **more degraded than
> the published levels**. This is the largest open risk in the submission.
...
under heavy noise is the same one as at low noise, only amplified.

> [!NOTE]
> **The confidence score degrades honestly.** AP tracks accuracy all the
> way down, from 1.000 to 0.149. It does not remain falsely high once the
> method stops working — which is the property that matters for a tool
...
> [!IMPORTANT]
> The last is the strongest. Weights derived from the **correct answer
> itself** — the best any attention or weighting scheme could achieve —
> recover almost nothing.
>
> That result bounds an entire family of methods, including a learned
> spatial verifier, and it is why the shipped method contains no model.
...
| 2 | Raster-drift correction on the output coordinate | no change — offset is symmetric across same-row candidates |
| 3 | Two-band disambiguation, low band selects the repeat | −5 pp |
| 4 | Fixed-width centre tie-break | **−15 pp** · see [§9.2](#92-the-centre-tie-break) |
...
| 16 | Candidate pruning before the resampling sweep | 80 % vs 96 % — truth is often ranked outside the top 30 |

<details>
<summary><b>Two of these are worth expanding</b></summary>
...
statistically significant gain. But the pipeline must beat *every* impostor
simultaneously, not one, and in competition it ranked the truth first 59
times in 91 against correlation's 75. End-to-end accuracy fell from 75 % to
59 %.

> **Pairwise separability does not survive the multi-candidate maximum.**
> That is the most transferable lesson in this document.

**#16 — pruning.** Restricting the resampling sweep to the top 30 peaks
from plain correlation costs **16 points**. The truth is ranked outside
that set in exactly the cases the sweep rescues, so pruning discards the
answer before the sweep can find it.

The cost of the full search is therefore structural, not an optimisation
failure.
...
**"Signal exists in the failures, 19 of 19."**
True and predicted regions were compared on the noise-free canvas and found
to differ by ~24 grey levels. The comparison did not account for lattice
phase: a lattice shifted by a non-period amount differs in pixels while
being the identical object.

**"Residual matching gains 12.5 points."**
Measured at n = 24, which is 1.25 standard errors. It lost 3 points on the
reference generator's data.

**"The impostor wins in 85 % of blocks."**
The comparison gave the impostor its optimal alignment and the true site
its ground-truth position — which the registration check showed is
typically 1 px from where the template actually aligns. Under fair
comparison the figure is 36 %.

Two errors in the analysis itself were also found and corrected: a **sign
error** in the phase experiment, which produced a false null; and a
**standard error computed at *p* = 0.5** when the true proportion was
≈ 0.97, which inflated the uncertainty threefold and briefly hid a real
effect.
...
~1.1 s per 1000 × 1000 pair against 47 ms for plain template matching —
**23×**. The method evaluates 25 resampling offsets over the full
correlation surface.

Pruning the position search was measured and rejected ([§7 #16](#7--rejected-approaches)). No time
limit is stated in the problem statement, and the method loads no model.

## 9.2 The centre tie-break

> [!WARNING]
> The two published descriptions of this rule **disagree**. The Applied
> Materials deck says closest to the *reference* image centre; the i4C
> statement says closest to the *search* image centre.
...
Accuracy falls to 68 % at *high* and 52 % at *severe* ([§5](#5--degradation-under-noise)). Since the
test set is stated to be noisier than the published levels, this is the
largest open risk. The confidence score continues to track accuracy through
the degradation, so failures remain identifiable even where they become
common.
...
`experiments/metrics.py` · `experiments/make_plots.py` · `experiments/make_figures.py`
